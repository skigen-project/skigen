...
$$\min_{\{\mu_k\}} \sum_{k=1}^{K} \sum_{x_i \in C_k} \|x_i - \mu_k\|_2^2$$

where $\mu_k$ is the centroid of cluster $C_k$. Lloyd's algorithm alternates between assigning each sample to its nearest centroid and recomputing centroids as cluster means.
